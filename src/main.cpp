...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dot: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dot Signed Message:\n";
...
{
    // Dot: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dot: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Dot: 2 days
static const int64 nTargetSpacing = 5 * 60; // Dot: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dot: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dot: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Dot: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "March 03, 2014. The creation of DOT.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dot: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DotMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DotMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DotMiner(CWallet *pwallet)
{
    printf("DotMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dot-miner");
    ...
    try { loop {
        ...
        printf("Running DotMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DotMiner, pwallet));
}
